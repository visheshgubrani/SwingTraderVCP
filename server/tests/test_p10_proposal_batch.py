...
class TestProposalBatchDeadline(unittest.IsolatedAsyncioTestCase):
    ...
    def test_auto_batch_deadline_resolves_to_next_session_approval_deadline(self) -> None:
        as_of_date = dt.date(2026, 8, 18)  # Tuesday
        deadline = proposal_batch_deadline(
            as_of_date=as_of_date,
            now=self.now,
            manual=False,
        )
        # Next session is 2026-08-19, approval deadline is 09:00 IST (03:30 UTC)
        expected = dt.datetime(2026, 8, 19, 9, 0, tzinfo=ZoneInfo("Asia/Kolkata"))
        self.assertEqual(deadline, expected)
        self.assertGreater(deadline, self.now)

    def test_manual_batch_on_past_date_provides_24h_window(self) -> None:
        as_of_date = dt.date(2026, 8, 18)
        past_deadline_now = dt.datetime(2026, 8, 20, 10, 0, tzinfo=dt.timezone.utc)
        deadline = proposal_batch_deadline(
            as_of_date=as_of_date,
            now=past_deadline_now,
            manual=True,
        )
        self.assertEqual(deadline, past_deadline_now + dt.timedelta(hours=24))
